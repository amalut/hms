...
<?php
    ...
    function login($email_id_unsafe, $password_unsafe, $table = 'patients')
    {
        global $connection;
        ...
        if ($num_rows > 1) {
            ...
            if ($table == 'patients' || $table == 'doctors' || $table == 'clerks') {
                $sql = "SELECT * FROM $table WHERE email = '$email_id' AND password = '$password';";
                ...
                if ($table == 'patients') {
                    ...
                    $_SESSION['email']=$email;
                    $_SESSION['fullname'] = $fullname;
                    $_SESSION['speciality'] = $specialist;
    ...
    function register($email_id_unsafe, $password_unsafe, $full_name_unsafe, $speciality_unsafe = 'doctor', $table = 'patients')
    {
        ...
        switch ($table) {
            case 'patients':
                ...
                break;
            case 'doctors':
                $sql = "INSERT INTO $table (email,password,fullname,speciality) VALUES ('$email', '$password', '$fullname','$speciality');";
                break;
            case 'clerks':
                $sql = "INSERT INTO $table (email,password,fullname) VALUES ('$email', '$password', '$fullname');";
                break;
            default:
                // code...
                break;
  ...
  function enter_patients($full_name_unsafe, $age_unsafe,$gender_unsafe,$weight_unsafe, $phone_no_unsafe, $address_unsafe)
  {
      ...
      $sql = "INSERT INTO `patients` VALUES ('$full_name', $age,'$gender',$weight, '$phone_no','$address');";
    ...
    function appointment_booking($patient_id_unsafe, $specialist_unsafe, $medical_condition_unsafe)
    {
        ...
        $sql = "INSERT INTO appointments (patient_id,speciality,ap_date,medical_condition)VALUES ('$patient_id', '$specialist',CURDATE(),'$medical_condition')";
    ...
    function getPatientsFor($doctor = 'Dentist')
    {
        ...
        return $connection->query("SELECT appointment_no, fullname, medical_condition,ap_date FROM patients, appointments where speciality='$doctor' AND patients.patient_id = appointments.patient_id");
    }
    ...
    function getAllAppointments()
    {
        ...
        return $connection->query('SELECT appointment_no, fullname,speciality, medical_condition FROM patients, appointments where patients.patient_id = appointments.patient_id');
    }
    ...
    function getAllPatientDetail($appointment_no)
    {
        ...
        return $connection->query("SELECT * FROM patients, appointments where appointment_no=$appointment_no AND patients.patient_id = appointments.patient_id;");
    }
    ...
    function get_table($purpose, $data)
    {
        ...
        switch ($purpose) {
            case 'patient_information':
                $sql = 'SELECT * FROM patients AND (SELECT )';
                break;
            case 'doctor-home':
                $sql = '';
